...
*   **Additional Features:**
    *   **App Links:** Deep linking functionality for sharing and joining groups via invite links.
    *   **Share Plus:** For sharing invite links across platforms.
...
*   **Schema:**
    *   `groupName` (String): The user-defined name for the transaction group (e.g., "Ski Trip 2024", "Apartment Bills").
    *   `owner` (String): The Firebase User UID of the user who created the group.
    *   `ownerName` (String): The display name of the user who created the group.
    ...
    *   `defaultTax` (Number): Default tax percentage for transactions in this group (defaults to 0.0).
    *   `defaultServiceCharge` (Number): Default service charge percentage for transactions in this group (defaults to 0.0).
...
*   **Schema:**
    ...
    *   `currency` (String): The symbol of the currency in which the transaction was made (should match a `symbol` in the group's `currency_rates`).
...
*   **Google Sign-In:** Users can sign up or sign in to the app using their Google accounts. This process is handled by Firebase Authentication, with all authentication logic managed in the `AppState` class.
*   **Automatic Public Profile Creation:** Upon a new user's first successful sign-in, a Firebase Cloud Function (`createPublicProfile`) is triggered. This function creates a document in the `publicProfiles` collection in Firestore, storing the user's ID, display name, photo URL, and creation timestamp. This profile is used to display user information within the app where needed (e.g., identifying who created a group).
*   **Deep Link Integration:** After successful authentication, the app automatically processes any pending invite tokens from deep links.
...
### 7. Deep Linking and App Links
*   **Invite Link Sharing:** Users can share transaction groups via deep links that contain invite tokens.
*   **Link Format:** `https://splitter-2e1ae.web.app/join?token={inviteToken}`
*   **Automatic Joining:** When a user clicks an invite link:
    *   If logged in: Automatically attempts to join the group
    *   If not logged in: Stores the pending invite token and prompts for sign-in
*   **Cross-Platform Sharing:** Uses the `share_plus` package to share invite links across different platforms and apps.
...
*   **Responsibilities:**
    *   **User Authentication State:** Manages the current Firebase `User` object and listens to authentication state changes (`_auth.authStateChanges()`). All authentication logic is handled directly in `AppState` rather than a separate service.
    *   **Deep Link Handling:** Manages `pendingInviteToken` for handling invite links when users are not logged in.
    ...
    *   **Business Logic Interface:** Provides methods that UI components can call to perform actions like adding/removing data (transactions, participants, etc.), signing in/out, and calculating settlements. These methods interact directly with Firebase services (Firestore, Cloud Functions).
...
## Application Entry Point and Deep Linking

The application's entry point (`lib/main.dart`) includes sophisticated deep linking functionality:

*   **Firebase Initialization:** Initializes Firebase services before starting the app.
*   **App Links Handler:** `AppWithLinkHandler` class manages incoming deep links:
    *   Listens to the `uriLinkStream` for incoming links
    *   Handles links with the format `https://splitter-2e1ae.web.app/join?token={inviteToken}`
    *   Automatically processes invite tokens for logged-in users
    *   Stores pending tokens for users who need to sign in first
*   **Global Scaffold Messenger:** Provides app-wide access to snackbar notifications for user feedback.
*   **Provider Setup:** Wraps the entire app with `ChangeNotifierProvider` to provide `AppState` throughout the widget tree.
...
The application uses the following key dependencies (as defined in `pubspec.yaml`):
...
*   **Core Flutter & Firebase:**
    ...
    *   `cloud_functions`: Firebase Cloud Functions client
...
*   **Additional Features:**
    *   `app_links`: Deep linking and app links handling
...
*   **`lib/`:**
    ...
    *   `main.dart`: The entry point of the Flutter application, includes deep linking setup.
    *   `firebase_options.dart`: Auto-generated Firebase configuration file.
    *   `models/`: Contains Dart classes representing the data structures used in the app:
        *   `transaction_group.dart`: `SplitterTransactionGroup` model
        *   `transaction.dart`: `SplitterTransaction` model
        *   `participant.dart`: `Participant` model
        *   `public_profile.dart`: `PublicProfile` model
        *   `currency_rate.dart`: `CurrencyRate` model
    *   `providers/`: Holds state management classes:
        *   `app_state.dart`: Main `AppState` class with all business logic
        *   `utils.dart`: Provider utility functions
    *   `screens/`: Contains the Dart files for different UI screens of the application.
    *   `services/`: Includes specific service classes:
        *   `settlement_service.dart`: For calculating settlements between participants
        *   `auth_service.dart`: Authentication service (though auth logic is primarily in AppState)
    *   `dialogs/`: UI components for various dialog boxes:
        *   `add_currency_dialog.dart`: Dialog for adding currency rates
        *   `add_transaction_group_dialog.dart`: Dialog for creating new groups
        *   `join_transaction_group_dialog.dart`: Dialog for joining groups via invite token
    *   `utils/`: Utility functions and helper classes:
        *   `input_utils.dart`: Input validation and sanitization utilities
...
*   **`functions/`:**
    ...
    *   `tsconfig.json` & `tsconfig.dev.json`: TypeScript compiler configurations.
...
*   **`test/`:**
    ...
    *   `settlement_service_test.dart`: Tests for the settlement calculation logic.
